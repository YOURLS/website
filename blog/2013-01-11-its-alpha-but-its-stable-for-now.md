...
This said, expect some breakages in the future: I'll slightly refactor the [way action works](https://code.google.com/p/yourls/issues/detail?id=1203), I'll change a bit [API returns](https://code.google.com/p/yourls/issues/detail?id=1277), probably a few other things, and of course, there'll be DB upgrading which is always the scary operation of all :)
...
When I'll start implementing potentially breaker features, I'll change the version number to something more frightening such as "`1.6-alpha-dont-use`", so be sure to [check it](https://yourls.org/version) before you update on a live setup.
